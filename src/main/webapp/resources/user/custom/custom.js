							...
							if (request.readyState === 4
								...
								if (request.responseText == "0") {
									...
									document.getElementById("bageItem" + id)
											.remove();
									var price = parseInt(document
											.getElementById("price" + id).innerHTML);
									var totalPrice = parseInt(document
											.getElementById("billPrice").innerHTML);
									document.getElementById("billPrice").innerHTML = (totalPrice - price)
											.toFixed(0);
	...
	quantities[i]
			.addEventListener(
					...
					function(e) {
						...
						request.onreadystatechange = function() {
							if (request.readyState === 4
								...
								if (result[0] == "0") {
									...
									var price = parseInt(document
											.getElementById("singlePrice" + id).innerHTML);
									var discount = parseInt(document
											.getElementById("discount" + id).innerHTML);
									var newPrice = quantity
											* (price * (100 - discount) / 100);
									var lastPrice = parseInt(result[1]);
									var totalPrice = parseInt(document
											.getElementById("billPrice").innerHTML);
									document.getElementById("price" + id).innerHTML = newPrice
											.toFixed(0);
									document.getElementById("billPrice").innerHTML = (totalPrice
											- lastPrice + newPrice).toFixed(0);

									document.getElementById("badgeQuantity"
											+ id).innerHTML = quantity;
									document.getElementById("badgePrice" + id).innerHTML = newPrice
											.toFixed(0);
								}
							}
						}
						...
						request.open('GET',
								'/Store/shopping-cart/change-quantity?id=' + id
										+ "&quantity=" + quantity);
						request.send();
					});
}
...
if (buy != null)
	buy.addEventListener('click', function(e) {
		e.preventDefault();
		var id = this.getAttribute("data-id");
		var buyQuantity = document.getElementById("buyQuantity");
		if (buyQuantity != null) {
			var quantity = buyQuantity.value;
			window.location.href = "/Store/shopping-cart/add/" + id + "/"
					+ quantity;
		} else {
			alert("Sản phẩm tạm hết hàng. Mời quý khách quay lại sau.");
		}
	});